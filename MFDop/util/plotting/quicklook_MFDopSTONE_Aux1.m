function quicklook_MFDopSTONE_Aux1(ddop,tlim)
%
% quicklook_MFDopSTONE_Aux1(ddop)
%
% Generates a quick plot of MFDop Aux1 (vertical beam) data from STONE 2025,
...
% check inputs
if(length(ddop)>1)
  error('This code only accepts a single struct of data from the Aux1 head.  You tried to pass in an array of structs.')
end
if(~strcmp(ddop.headID,'Aux1'))
  error(['This code requires data from the Aux1 head.  You passed in a struct ' ...
...
ibeam=findCellStr(ddop.dopraw.beamname,'Aux_1H');
ifreq=1;  % just show freq1 data as representative
scaledIntensity = ( ddop.dopraw.r.*ddop.dopraw.Amp(:,:,ifreq,ibeam) ).^2;

% make some plots
clf
splotlist(1)=subplot(311);  % w
pcolor(tsec(indt),ddop.uvw.r(indr),ddop.uvw.w(indr,indt)),sf
caxis(wscale)
title('w [m/s]')
